...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let diff = currentTime - timeFromMessage;
      // check for 5 minutes
      if (diff > EXPIRATION) {
    ...
    return new Promise((resolve, reject) => {
      self.chain.forEach(async (block, index) => {
        ...
        if (index > 0) {
          ...
          let previousBlockHashFromCurrentBlock = block.previousBlockHash;
          if (previousBlockHash !== previousBlockHashFromCurrentBlock) {
            errorLog.push("Chain tampered at:" + block.height);
          }
        }
      });
